      ...
      <div className="container mx-auto px-6 lg:px-12">
        ...
        once: true
          ...
          <div className="max-w-2xl mb-8 lg:mb-0">
            ...
            <p className="text-muted-foreground text-lg font-light">A curated selection of our finest new build projects across Portugal.</p>
          </div>

          {/* Filter Tabs */}
          <div className="flex flex-wrap gap-2">
            {filters.map(filter => <motion.button key={filter} onClick={() => setActiveFilter(filter)} className={`px-6 py-3 text-xs font-light tracking-widest uppercase transition-all duration-300 border ${activeFilter === filter ? 'bg-foreground text-background border-foreground' : 'bg-transparent text-foreground border-border hover:border-foreground'}`} whileHover={{
            scale: 1.02
          }} whileTap={{
            scale: 0.98
          }}>
